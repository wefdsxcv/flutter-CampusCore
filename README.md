...
resend登録　.env　にapikeyを設定。　resendはapiたたくだけでgmailなりが遅れる外部サービス。

本当はテスト用ではなく、resendに登録すれば　いろんなメールアドレスに送れる。メールアドレスは登録する時に　入力させているので、そこをうまく、とってきてjob.dataに入れる。で送るとか。あと通知処理するなら、otuboxパターンなど冪等性に考慮せよ。


今回は
テスト用のgmailを使った。本番サービスではここを変える。
